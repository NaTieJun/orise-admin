<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作功能" align="center" prop="title" width="100" />
      <el-table-column label="操作类型" align="center" prop="businessType" :formatter="typeFormat" width="80" />
      <el-table-column label="操作内容" prop="operParam">
        <template slot-scope="scope">
          <JsonViewer
            :value="JSON.parse(scope.row.operParam)"
            sort
            :expand-depth="0"
            :show-array-index="false"
            v-if="scope.row.operParam!='' && scope.row.operParam!='{}'"
          />
        </template>
      </el-table-column>
      <el-table-column label="操作结果" prop="jsonResult">
        <template slot-scope="scope">
          <JsonViewer :value="JSON.parse(scope.row.jsonResult)" sort :expand-depth="0" :show-array-index="false" />
        </template>
      </el-table-column>
...
<script>
import { listOperatorLog } from "@/api/system/log";
...
export default {
  ...
  methods: {
    ...
    fetchList() {
      this.loading = true;
      listOperatorLog(this.addDateRange(this.queryParams, this.dateRange)).then(response => {
