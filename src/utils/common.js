...
const systemURL = window.lwConfig.SYSTEM_API;
...
// 通用下载方法 系统内部
export function download(fileName, rename, path) {
  if (path == null) path = "/common/download";
  axios({
    method: "GET",
    url:
      systemURL + path + "?fileName=" + encodeURI(fileName) + "&delete=" + true,
    responseType: "blob",
    headers: {
...
// 通用下载方法 系统内部
export function downloadFullPath(path, fileRename) {
  axios({
    method: "GET",
    url: path,
    responseType: "blob",
    ...
    // if (rename == null) rename = fileRename;
    const { data, headers } = res;
    const blob = new Blob([data], { type: headers["content-type"] });
    let dom = document.createElement("a");
    let url = window.URL.createObjectURL(blob);
    dom.href = url;
    dom.download = fileRename;
    dom.style.display = "none";
    document.body.appendChild(dom);
    dom.click();
    dom.parentNode.removeChild(dom);
...
// 通用下载方法 系统内部
export function downloadByPath(fileName, rename) {
  this.downloadFullPath(fileName, rename);
}

// 通用下载方法 系统内部
export function downloadByParams(path, params = {}, fileName, data) {
  let method = "GET";
  if (data != null) {
    method = "POST";
  }
  ...
  const url = systemURL + path + "?" + arr.join("&");
  return axios({
    method: method,
    url: url,
    data: data,
